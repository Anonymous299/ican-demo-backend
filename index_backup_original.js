...
// Class Management CRUD Operations (Admin only)
app.post('/api/classes', authenticateToken, (req, res) => {
  ...
  const { 
    grade, division, description, teacherId, classTeacher, capacity, 
    subjects, schedule, academicYear 
  } = req.body;
  
  // Validate required fields
  if (!grade || !division) {
    return res.status(400).json({ error: 'Grade and division are required' });
  }
  
  // Check for duplicate class
  const existingClass = data.classes.find(c => 
    c.grade === grade && c.division === division && c.academicYear === academicYear
  );
  
  if (existingClass) {
    return res.status(400).json({ 
      error: `Class ${grade}${division} already exists for academic year ${academicYear}` 
    });
  }
  
  // Calculate current enrollment from students
  const currentEnrollment = data.students.filter(s => 
    s.standard === grade && s.division === division
  ).length;
  
  const newClass = {
    id: Date.now(),
    name: `Grade ${grade}${division}`,
    grade,
    division,
    description: description || `Class ${grade}${division}`,
    teacherId: teacherId || null,
    classTeacher: classTeacher || null,
    capacity: capacity || 20,
    currentEnrollment,
    subjects: subjects || ['Mathematics', 'English', 'Science', 'Social Studies'],
...
app.get('/api/classes/:id', authenticateToken, (req, res) => {
  ...
  // Add enrolled students information
  const enrolledStudents = data.students.filter(s => 
    s.standard === classInfo.grade && s.division === classInfo.division
  );
...
app.put('/api/classes/:id', authenticateToken, (req, res) => {
  ...
  const { 
    grade, division, description, teacherId, classTeacher, capacity,
    subjects, schedule, academicYear, status
  } = req.body;
  
  // Check for duplicate if grade/division changed
  if (grade && division) {
    const existingClass = data.classes.find(c => 
      c.id !== classId && c.grade === grade && c.division === division && 
      c.academicYear === (academicYear || data.classes[classIndex].academicYear)
    );
    
    if (existingClass) {
      return res.status(400).json({ 
        error: `Class ${grade}${division} already exists for this academic year` 
      });
    }
  }
  ...
  if (classTeacher !== undefined) updates.classTeacher = classTeacher;
  ...
  if (updates.grade !== undefined || updates.division !== undefined) {
    const newGrade = updates.grade || data.classes[classIndex].grade;
    const newDivision = updates.division || data.classes[classIndex].division;
    updates.name = `Grade ${newGrade}${newDivision}`;
  }
  
  // Recalculate enrollment if grade/division changed
  if (updates.grade !== undefined || updates.division !== undefined) {
    const newGrade = updates.grade || data.classes[classIndex].grade;
    const newDivision = updates.division || data.classes[classIndex].division;
    updates.currentEnrollment = data.students.filter(s => 
      s.standard === newGrade && s.division === newDivision
    ).length;
  }
  
  updates.updatedAt = new Date().toISOString();
  
  Object.assign(data.classes[classIndex], updates);
...
app.delete('/api/classes/:id', authenticateToken, (req, res) => {
  ...
  const classToDelete = data.classes[classIndex];
  
  // Check if class has enrolled students
  const enrolledStudents = data.students.filter(s => 
    s.standard === classToDelete.grade && s.division === classToDelete.division
  );
  
  if (enrolledStudents.length > 0) {
    return res.status(400).json({ 
      error: `Cannot delete class with ${enrolledStudents.length} enrolled students. Please transfer students first.`,
...
app.get('/api/classes/:id/stats', authenticateToken, (req, res) => {
  const classId = parseInt(req.params.id);
  const classInfo = data.classes.find(c => c.id === classId);
  ...
  const stats = {
    ...
    availableSpots: classInfo.capacity - enrolledStudents.length,
    enrollmentPercentage: Math.round((enrolledStudents.length / classInfo.capacity) * 100),
    subjects: classInfo.subjects.length,
    hasClassTeacher: !!classInfo.classTeacher,
...
app.post('/api/classes/bulk', authenticateToken, (req, res) => {
  if (req.user.role !== 'admin') {
    return res.status(403).json({ error: 'Admin access required' });
  }
  
  const { operation, classes: classData } = req.body;
  
  if (operation === 'create_standard_classes') {
    // Create default classes for a grade (A-F divisions)
    const { grade, academicYear = '2024-25' } = classData;
    ...
    divisions.forEach(division => {
      ...
      if (!existingClass) {
        const newClass = {
          ...
          grade,
          division,
          description: `Grade ${grade} Division ${division}`,
          teacherId: null,
          classTeacher: null,
          capacity: 20,
          currentEnrollment: 0,
        ...
        errors.push(`Class ${grade}${division} already exists`);
      }
    });
